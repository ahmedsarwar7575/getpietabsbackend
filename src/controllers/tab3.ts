...
type UploadedFiles = {
    frontBusinessPicture?: Express.Multer.File[];
    businessLogo?: Express.Multer.File[];
};

export const createTab3 = async (req: Request, res: Response) => {
    try {
        const {
            name,
            email,
...
export const getTab3 = async (req: Request, res: Response) => {
    try {
...
export const getTab3ById = async (req: Request, res: Response) => {
    const { id } = req.params
    try {
...
export const updateTab3 = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        ...
        if (!tab3) return res.status(404).json({ message: "Tab3 not found" });
...
// Delete API
export const deleteTab3 = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const tab3 = await Tab3.findByPk(id);
        if (!tab3) return res.status(404).json({ message: "Tab3 not found" });
