...
// Create API
export const createTab2 = async (req, res) => {
    try {
...
export const getTab2 = async (req, res) => {
    try {
...
export const updateTab2 = async (req, res) => {
    try {
        const { id } = req.params;
        const { description } = req.body;
        ...
        if (!tab2)
            return res.status(404).json({ message: "Tab2 not found" });
...
// Delete API
export const deleteTab2 = async (req, res) => {
    try {
        const { id } = req.params;
        const tab2 = await Tab2.findByPk(id);
        if (!tab2)
